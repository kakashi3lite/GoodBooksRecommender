...
from src.models.news_models import BookRecommendation, FactCheck, NewsItem
...
class TestNewsExpansionAPI:
    ...
    @pytest.mark.asyncio
    async def test_expand_news_fact_hunter_failure(
        self, news_api, sample_news_item, mock_fact_hunter
    ):
        """Test graceful handling of fact hunter failure."""
        mock_fact_hunter.verify_facts.side_effect = NetworkError(
        ...
        assert result.facts == []
        assert (
            len(result.book_recommendations) > 0
        )  # Book recommendations should still work
...
class TestFactHunterService:
    ...
    async def test_multiple_sources_verification(self, fact_hunter):
        ...
        with patch.object(fact_hunter, "_check_wikipedia") as mock_wiki, patch.object(
            fact_hunter, "_check_reuters"
        ) as mock_reuters, patch.object(fact_hunter, "_check_snopes") as mock_snopes:
...
class TestContextBookRecommender:
    ...
    async def test_recommend_by_topic(self, book_recommender):
        ...
        assert any(
            "AI" in rec.title or "artificial" in rec.title.lower()
    ...
    async def test_recommend_by_content(self, book_recommender):
        ...
        assert any(
            "Technology" in genre or "Science" in genre
            for rec in recommendations
            for genre in rec.genre
        )
    ...
    async def test_recommendation_diversity(self, book_recommender):
        ...
        recommendations = await book_recommender.get_recommendations_by_topic(
            topic, num_recommendations=10
        )
