...
from src.news.api.news_expansion import router, NewsExpansionRequest, NewsExpansionResponse
from src.news.services.fact_hunter import FactHunterEngine, FactCheck
from src.news.services.context_book_recommender import ContextBookRecommender, BookRecommendation
from src.news.core.intelligence_engine import NewsIntelligenceEngine, NewsArticle
...
class TestFactHunterEngine:
    ...
    async def test_claim_extraction_basic(self, fact_hunter, sample_article_content):
        ...
        claims = await fact_hunter.extract_claims(sample_article_content, "AI Study Results")
    ...
    async def test_claim_extraction_empty_content(self, fact_hunter):
        ...
        claims = await fact_hunter.extract_claims("The weather is nice today.", "Opinion Piece")
    ...
    async def test_verify_claims_with_mock_sources(self, mock_get, fact_hunter):
        ...
        claims = [
            {"text": "Harvard University conducted the research", "type": "institutional"},
            {"text": "95% improvement in learning outcomes", "type": "statistical"}
        ]
        
        fact_checks = await fact_hunter.verify_claims("Sample content", "AI Study", claims)
    ...
    async def test_claim_type_classification(self, fact_hunter):
        ...
        for claim_text, expected_type in test_claims:
            ...
            assert claim_type == expected_type, f"Claim '{claim_text}' classified as {claim_type}, expected {expected_type}"
...
class TestContextBookRecommender:
    ...
    async def test_topic_matching_algorithm(self, book_recommender, sample_books_database):
        """Test the core topic matching algorithm"""
        # Mock the book database
        with patch.object(book_recommender, 'get_books_database', return_value=sample_books_database):
    ...
    async def test_relevance_scoring_accuracy(self, book_recommender, sample_books_database):
    ...
    async def test_recommendation_diversity(self, book_recommender, sample_books_database):
        ...
        with patch.object(book_recommender, 'get_books_database', return_value=extended_database):
            recommendations = await book_recommender.get_context_recommendations(
                topics=["climate"],
        ...
        assert len(set(ratings)) > 1, "Recommendations should include diverse quality levels"
...
class TestNewsExpansionAPI:
    ...
    @patch('src.news.services.context_book_recommender.ContextBookRecommender.get_context_recommendations')
    async def test_expand_news_story_success(self, mock_books, mock_facts, mock_article_fetch, client, sample_expansion_request, mock_article):
    ...
    @patch('src.news.core.intelligence_engine.NewsIntelligenceEngine.get_article_by_id')
    async def test_expand_news_story_not_found(self, mock_article_fetch, client, sample_expansion_request):
    ...
    @patch('src.news.core.intelligence_engine.NewsIntelligenceEngine.get_article_by_id')
    async def test_expansion_caching(self, mock_article_fetch, mock_cache_get, client, sample_expansion_request, mock_article):
    ...
    async def test_quick_expand_endpoint(self, client):
        ...
        with patch('src.news.core.intelligence_engine.NewsIntelligenceEngine.get_article_by_id') as mock_fetch:
            ...
            response = client.get("/api/news/expand/quick-test-123?include_facts=true&include_books=false")
            assert response.status_code == 200
...
class TestPerformanceAndScaling:
    ...
    async def test_response_time_requirements(self, performance_client):
        ...
        with patch('src.news.core.intelligence_engine.NewsIntelligenceEngine.get_article_by_id') as mock_fetch, \
             patch('src.news.services.fact_hunter.FactHunterEngine.verify_claims') as mock_facts, \
             patch('src.news.services.context_book_recommender.ContextBookRecommender.get_context_recommendations') as mock_books:
    ...
    async def test_concurrent_request_handling(self, performance_client):
        ...
        with patch('src.news.core.intelligence_engine.NewsIntelligenceEngine.get_article_by_id') as mock_fetch:
            ...
            async with httpx.AsyncClient(app=app, base_url="http://test") as client:
                tasks = [make_request(client, f"concurrent-test-{i}") for i in range(20)]
            ...
            assert max(response_times) < 1000, f"Max response time {max(response_times)}ms under concurrent load"
            assert sum(response_times) / len(response_times) < 500, "Average response time should be under 500ms"
    ...
    async def test_cache_performance_impact(self, performance_client):
        ...
        with patch('src.core.cache.AsyncCacheManager.get') as mock_cache_get, \
             patch('src.core.cache.AsyncCacheManager.set') as mock_cache_set:
            
            # First request (cache miss)
            mock_cache_get.return_value = None
            
            start_time = time.time()
            response1 = performance_client.post("/api/news/expand", json=request_payload)
            ...
            start_time = time.time()
            response2 = performance_client.post("/api/news/expand", json=request_payload)
            ...
            assert performance_improvement > 0.5, f"Cache should improve performance by >50%, got {performance_improvement:.2%}"
...
class TestErrorHandlingAndResilience:
    ...
    async def test_all_ai_services_failure(self, resilience_client):
        ...
        with patch('src.news.core.intelligence_engine.NewsIntelligenceEngine.get_article_by_id') as mock_fetch:
            ...
            with patch('src.news.services.fact_hunter.FactHunterEngine.verify_claims') as mock_facts, \
                 patch('src.news.services.context_book_recommender.ContextBookRecommender.get_context_recommendations') as mock_books, \
                 patch('src.news.core.intelligence_engine.NewsIntelligenceEngine.find_related_articles') as mock_related:
    ...
    async def test_database_connection_failure(self, resilience_client):
        ...
        request_payload = {
            "article_id": "db-failure-test-123"
        }
        ...
        with patch('src.news.core.intelligence_engine.NewsIntelligenceEngine.get_article_by_id') as mock_fetch:
            ...
            assert "Database" in response.json()["detail"] or "connection" in response.json()["detail"].lower()
