...
import uvicorn
from fastapi import FastAPI, HTTPException, Depends, Request, Response, BackgroundTasks, status
...
from src.core.enhanced_logging import StructuredLogger, get_correlation_id, set_correlation_id
from src.core.monitoring import MetricsCollector, PerformanceMonitor, start_background_monitoring
from src.core.tracing import TracingManager, trace_operation, get_tracer
from src.core.exceptions import (
    GoodBooksException, RecommendationError, ValidationError,
    RateLimitError, AuthenticationError, CacheError
)
from src.core.cache import AsyncCacheManager
from src.core.vector_store import BookVectorStore, VectorStoreError
from src.core.session_store import RedisSessionStore, UserInteraction, SessionStoreError

# Security and Authentication modules
from src.auth.security import (
    OAuth2Manager, RBACManager, get_current_user, get_current_active_user,
    require_roles, require_permissions, User, TokenData, UserRole
)
from src.middleware.security_middleware import (
    SecurityMiddleware, CSPMiddleware, InputValidationMiddleware,
    SecurityHeadersMiddleware, RateLimitingMiddleware
)
from src.privacy.data_privacy import (
    DataPrivacyService, AnonymizationLevel, RetentionPolicy
)
...
from src.services.rag_service import RAGExplanationService, RAGError
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        with trace_operation("recommender_training", {"component": "hybrid_recommender"}):
...
async def get_api_key(api_key: Optional[str] = Depends(api_key_header)) -> str:
    ...
    valid_keys = settings.security.api_keys if hasattr(settings.security, 'api_keys') else [settings.security.default_api_key]
...
async def initialize_data_and_models():
    ...
    try:
        ...
        merged_books = await data_loader.merge_book_metadata_async(books, book_tags, tags)
...
def get_recommender_for_request(user_id: int, experiment_id: Optional[str] = None) -> HybridRecommender:
    """
...
async def record_recommendation_metrics(user_id: int, 
...
async def ab_testing_middleware(request: Request, call_next):
    ...
    experiment_id = request.headers.get('X-Experiment-ID') or request.query_params.get('experiment_id')
...
if not settings.is_testing:
    ...
    app.add_middleware(
        ...
        allowed_hosts=["*"] if settings.is_development else settings.security.allowed_hosts
    )
...
async def verify_api_key(api_key: Optional[str] = Depends(api_key_header)) -> Optional[str]:
    ...
    logger.warning("API key authentication is deprecated. Please use OAuth2/JWT tokens.")
...
class RecommendationRequest(BaseModel):
    ...
    book_title: Optional[str] = Field(None, description="Book title for content-based filtering")
...
class RecommendationResponse(BaseModel):
    ...
    recommendations: List[BookRecommendation] = Field(..., description="List of recommendations")
    total_count: int = Field(..., description="Total number of recommendations")
    processing_time_ms: float = Field(..., description="Processing time in milliseconds")
    cache_hit: bool = Field(..., description="Whether result was served from cache")
    explanation: Optional[Dict[str, Any]] = Field(None, description="Global explanation")
...
class HealthCheckResponse(BaseModel):
    ...
    health_check_duration_ms: float = Field(..., description="Health check duration in milliseconds")
...
class ExplainRequest(BaseModel):
    ...
    n_context_books: int = Field(
        ...
        ge=1,
        le=10,
        description="Number of similar books for context"
    )
...
class ExplainResponse(BaseModel):
    ...
    book_info: Dict[str, Any] = Field(..., description="Information about the queried book")
    processing_time_ms: float = Field(..., description="Processing time in milliseconds")
...
class SearchRequest(BaseModel):
    ...
    include_explanation: bool = Field(False, description="Include explanation for results")
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "explanation": {
                    "text": "Found books matching fantasy themes..."
                }
            }
        }
    ...
    with tracer.start_as_current_span(
        ...
        try:
            ...
            logger.error(
                ...
                path=request.url.path,
                error_type=type(e).__name__,
                error_message=str(e),                duration_ms=duration * 1000,
                exc_info=True
            )
...
async def health_check():
    ...
    try:
        if recommender and hasattr(recommender, 'is_fitted') and recommender.is_fitted:
            ...
            test_recs = await recommender.get_recommendations(user_id=1, n_recommendations=1)
...
async def get_recommendations(
    ...
    try:
        ...
        current_recommender = get_recommender_for_request(effective_user_id, experiment_id)
        ...
        if cached_result:
            ...
            if experiment_id:
                background_tasks.add_task(
                    ...
                    anonymized_user_id, cached_result.get('recommendations', []), 
                    experiment_id, 'cache_hit'
                )
        ...
        if experiment_id:
            background_tasks.add_task(
                record_recommendation_metrics,
                anonymized_user_id, recommendations, experiment_id, 'recommendation_generated'
            )
        ...
        if experiment_id:
            ...
            assignment = ab_tester.experiments.get(experiment_id, {}).get('assignments', {}).get(effective_user_id, 'control')
...
async def create_experiment(
    ...
    try:
        ...
        metrics = experiment_request.get('metrics', ['precision', 'recall', 'click_through_rate'])
...
async def rebuild_vector_store(
    ...
    try:
        logger.info(
            "Vector store rebuild requested",
            admin_user=current_user.username
        )
        ...
        if ab_tester:
            ab_health = {
                ...
                "active_experiments": len([
                    exp for exp in ab_tester.experiments.values() 
