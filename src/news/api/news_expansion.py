...
class NewsExpansionResponse(BaseModel):
    ...
    credibility_score: float = Field(
        ..., ge=0, le=1, description="Credibility assessment"
    )

    # Expansion components
    fact_checks: List[FactCheck] = Field(
        default=[], description="Fact verification results"
    )
    book_recommendations: List[BookRecommendation] = Field(
        default=[], description="Relevant books"
    )
    related_articles: List[RelatedArticle] = Field(
        default=[], description="Related news"
    )
...
@router.post(
    "/expand",
    response_model=NewsExpansionResponse,
...
async def expand_news_story(
    ...
    try:
        ...
        if not article:
            raise HTTPException(
                status_code=404, detail="Article not found or could not be fetched"
            )
        ...
        summary = await news_engine.generate_summary(
            article.content, length=summary_length_mapping[request.summary_level]
        )
        ...
        if request.include_related:
            expansion_tasks.append(
                news_engine.find_related_articles(
                    article_content=article.content, topics=topics, limit=5
                )
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to expand news story: {str(e)}"
        )


@router.get(
    "/stories/trending",
    response_model=List[NewsExpansionResponse],
    summary="Get trending expandable stories",
)
async def get_trending_expandable_stories(
    ...
    try:
        ...
        articles = await news_engine.get_trending_articles(
            limit=limit, category=category, time_window_hours=24
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch trending stories: {str(e)}"
        )
...
@router.get(
    "/expand/{article_id}",
    response_model=NewsExpansionResponse,
    summary="Quick expand by article ID",
)
