...
class FactHunterEngine:
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(
            ...
            headers={"User-Agent": "GoodBooks-FactHunter/1.0 (Fact Verification Bot)"},
        )
        return self
                        ...
                        fact_checks.append(
                            {
                                ...
                                "sources": [
                                    source.url for source in verification.sources
                                ],
        ...
        try:
            ...
            for sentence in sentences:
                ...
                if any(
                    ...
                    for word in ["study", "research", "data", "report"]
                ):
                    confidence += 0.1
    ...
    async def _verify_single_claim(self, claim: FactClaim) -> FactVerification:
        ...
        try:
            ...
            verdict, confidence, explanation = await self._analyze_sources(
                claim, sources
            )
            ...
            logger.error(
                "Single claim verification failed", claim=claim.text, error=str(e)
            )
            return FactVerification(
                claim=claim.text,
                verdict="Unverified",
                    ...
                    if data.get("AbstractSource"):
                        sources.append(
                            FactSource(
                                ...
                                credibility_score=self._get_domain_credibility(
    ...
    async def _analyze_sources(
        self, claim: FactClaim, sources: List[FactSource]
    ) -> Tuple[str, float, str]:
        ...
        try:
            ...
            if avg_credibility >= 0.8:
                if len(sources) >= 2:
                    ...
                    explanation = (
                        "Supported by reliable source but needs additional verification"
                    )
