...
class ContextBookRecommender:
    ...
    def _initialize_topic_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "leadership": ["leadership", "management", "self-help", "personal development"],
        ...
        try:
            async with self:
                ...
                if not book_keywords:
                    logger.info("No relevant book keywords found for topics", topics=topics)
                    return []
        ...
        try:
            ...
            for topic in topics:
                ...
                for key, book_terms in self.topic_book_mapping.items():
                    if key in topic_lower or any(key_part in topic_lower for key_part in key.split()):
    ...
    def _extract_content_keywords(self, content: str) -> List[str]:
        ...
        try:
            ...
            important_terms = [
                "innovation", "research", "study", "analysis", "report",
                "expert", "scientist", "professor", "author", "leader",
                "strategy", "method", "approach", "solution", "future",
                "development", "growth", "change", "impact", "influence"
            ]
    ...
    async def _get_multi_approach_recommendations(
        ...
        try:
            ...
            keyword_recs = await self._get_keyword_based_recommendations(keywords, n_recommendations)
            recommendations.extend(keyword_recs)
            
            # Approach 2: Topic-based recommendations
            topic_recs = await self._get_topic_based_recommendations(topics, n_recommendations)
    ...
    async def _get_keyword_based_recommendations(self, keywords: List[str], limit: int) -> List[BookMatch]:
        ...
        try:
            ...
            for book in keyword_books:
                ...
                for keyword in keywords:
                    if any(keyword.lower() in book_keyword.lower() for book_keyword in book["keywords"]):
                ...
                if relevance_score > 0.1:  # Threshold for relevance
                    recommendations.append(BookMatch(
    ...
    async def _get_topic_based_recommendations(self, topics: List[str], limit: int) -> List[BookMatch]:
    ...
    async def _get_curated_recommendations(self, keywords: List[str], topics: List[str]) -> List[BookMatch]:
...
async def get_books_for_news(topics: List[str], content: str = "", limit: int = 5) -> List[Dict[str, Any]]:
